...
public class RoleController extends BaseController {	
   ...
   public String formAuthUser(Role role, Model model, HttpServletRequest request) {	
      ...
      model.addAttribute("rle", role);	
      return "modules/ss/roleFormAuthUser";	
   }	
   ...
   public List treeData(String userType, Boolean isAll, String isShowCode, String ctrlPermi) {	
      ...
      Role a = new Role();	
      a.setStatus("0");	
      a.setUserType(StringUtils.isNotBlank(userType) ? userType : "employee");	
      if (isAll == null || !isAll) {	
         this.roleService.addDataScopeFilter(a, (String)StringUtils.defaultIfBlank(ctrlPermi, "1"));	
      }	
   ...
   public String delete(Role role, HttpServletRequest request) {	
      if (Role.CORP_ADMIN_ROLE_CODE.equals(role.getRoleCode())) {	
         ...
         if ((a = (Role)super.getWebDataBinderSource(request)) != null && "1".equals(a.getIsSys()) && !role.getCurrentUser().isSuperAdmin()) {	
            return this.renderResult("false", "越权操作，只有超级管理员才能修改系统数据！");	
         } else if (this.roleService.hasUserRoleByRoleCode(role)) {	
            return this.renderResult("false", (new StringBuilder()).insert(0, "角色").append(role.getRoleName()).append("关联了用户，删除角色失败！").toString());	
         } else {	
   ...
   @ResponseBody	
   public Page listData(Role role, HttpServletRequest request, HttpServletResponse response) {	
      if (!role.getCurrentUser().isSuperAdmin()) {	
         this.roleService.addDataScopeFilter(role, "2");	
      }	
	
      return this.roleService.findPage(new Page(request, response), role);	
   }	
   ...
   public Map menuTreeData(Role role, HttpServletRequest request) {	
      ...
      while(var12.hasNext()) {	
         ...
         a.put("name", (new StringBuilder()).insert(0, a.getMenuName()).append("<font clr=#888> &nbsp; &nbsp; ").append(StringUtils.abbr((new StringBuilder()).insert(0, ObjectUtils.toString(a.getPermission())).append(" &nbsp; ").append(ObjectUtils.toString(a.getMenuHref())).toString(), 50)).append("</font>").toString());	

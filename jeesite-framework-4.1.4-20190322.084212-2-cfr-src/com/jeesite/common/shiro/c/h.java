...
import com.jeesite.common.shiro.c.m;	
...
extends m {	
    ...
    protected final boolean isPermitted(Permission permission, AuthorizationInfo info) {	
        try {	
            ...
            if (a != null) {	
                void a2;	
                String string = (String)com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("modules");	
                if (StringUtils.contains((CharSequence)a2, "," + a + ",") && !StringUtils.contains((CharSequence)((String)com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("openModules")), new StringBuilder().insert(0, ",").append(a).append(",").toString())) {	
    ...
    protected final AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {	
        ...
        if (!(authcToken instanceof FormToken) && !ObjectUtils.toBoolean(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("fnCas")).booleanValue()) {	
            return null;	
        }	
        ...
        if (!com.jeesite.common.mybatis.e.j.ALLATORIxDEMO((HttpServletRequest)(a = ServletUtils.getRequest()))) {	
            throw new AuthenticationException("msg:登录失败，请联系管理员获取许可！");	
        }	
        if (!(this.sessionDAO instanceof H) && !ObjectUtils.toBoolean(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("fnCluster")).booleanValue()) {	
            return null;	
        }	
        a4 = this.getUserInfo(a5);	
        if (a4 != null) {	
            if (("0".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) || "9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type"))) && (com.jeesite.common.mybatis.e.j.ALLATORIxDEMO() >= 3 + ("9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) ? 0 : 17) || l.ALLATORIxDEMO || "9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) && ManagementFactory.getRuntimeMXBean().getUptime() / 86400000L > 1L)) {	
                throw new AuthenticationException(new StringBuilder().insert(0, "msg:登录失败，当前登录的人数过多，").append("9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) ? "当前版本为开发版，请切换到正式版本再试！" : "请联系管理员！").append("谢谢使用！").toString());	
            }	
    ...
    public h() {	
        h h2 = this;	
        h2.setCachingEnabled(false);	
        h2.setAuthenticationTokenClass(FormToken.class);	
    }	
    ...
    protected final AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {	
        ...
        if (!Global.getPropertyToBoolean("shiro.isAllowMultiAddrLogin", "true").booleanValue() && (a3 = this.sessionDAO.getActiveSessions(false, true, a = ObjectUtils.toString(a10.getId()), null, a7.getId())).size() > 0) {	
            ...
            for (Session a422 : a3) {	
                ...
                if (a6 == null) continue;	
                if (Global.getPropertyToBoolean("shiro.isAllowMultiDeviceLogin", "true").booleanValue()) {	
        ...
        if (("0".equals(j.ALLATORIxDEMO().get("type")) || "9".equals(j.ALLATORIxDEMO().get("type"))) && (j.ALLATORIxDEMO() >= 4 + ("9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) ? 0 : 17) || I.ALLATORIxDEMO || "9".equals(com.jeesite.common.mybatis.e.j.ALLATORIxDEMO().get("type")) && ManagementFactory.getRuntimeMXBean().getUptime() / 86400000L > 1L)) {	
            return null;	
        }	

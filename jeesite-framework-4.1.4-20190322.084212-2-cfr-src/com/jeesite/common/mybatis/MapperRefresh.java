...
implements Runnable {	
    ...
    public void run() {	
        ...
        if (enabled) {	
            ...
            new Thread((Runnable)new m(this, a), "MyBatis-Mapper-Refresh").start();	
        }	
    }	
    ...
    extends HashMap<String, V> {	
        ...
        public V put(String key, V value) {	
            ...
            if (MapperRefresh.isRefresh()) {	
                this.remove(key);	
                log.debug(new StringBuilder().insert(0, "Mapper [").append(key).append("] already exists, is refreshed.").toString());	
            }	
            if ((a = super.get(key)) != null && value instanceof MappedStatement) {	
                ...
                if (a2 instanceof DynamicSqlSource) {	
                    ...
                    log.debug(new StringBuilder().insert(0, "Mapper [").append(key).append("] already exists, is ignored.").toString());	
                    return null;	
                }	
            }	
            if (key.contains(".")) {	
